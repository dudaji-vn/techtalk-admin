...
const InputController = <T extends FieldValues>(props: InputProps<T>) => {
  ...
  const renderInputElement = (inputProps: TInputElement) => {
    switch (type) {
      ...
      case 'textarea':
        return <textarea id={name} {...inputProps} placeholder={placeholder} className="w-full bg-blackBgCt p-[10px] text-whiteCt" />;
      case 'multiselect':
        return (
          <Multiselect
            options={options}
        ...
        return (
          <input
            ...
            className={overrideTailwindClasses(
              `w-full rounded-lg border-[1.5px] border-stroke bg-white py-3 px-5 font-medium outline-none transition focus:border-green active:border-green disabled:cursor-default  ${className}`
            )}
          />
        );
    }
